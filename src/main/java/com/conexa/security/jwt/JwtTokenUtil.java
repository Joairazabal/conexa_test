...
public class JwtTokenUtil {
    ...
    public String generateJwtToken(Authentication authentication, String rol) {
    ...
    public boolean validateJwtToken(String authToken) {
        try {
            ...
            log.error("JWT token expiradó: {}", e.getMessage());
        } catch (ExpiredJwtException e) {
            log.error("JWT Espiro el token: {}", e.getMessage());
        } catch (UnsupportedJwtException e) {
            log.error("JWT token no soportado: {}", e.getMessage());
        } catch (IllegalArgumentException e) {
            log.error("JWT claims vacios: {}", e.getMessage());
        }
